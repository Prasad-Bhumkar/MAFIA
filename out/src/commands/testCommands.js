...
function registerTestCommands(context, aiService) {
    const testGenerator = new TestGenerator_1.TestGenerator(aiService);
    const generateTestsCommand = vscode.commands.registerCommand('mafia.generateTests', async (uri) => {
        try {
            await testGenerator.generateTestsForFile(uri);
        }
        catch (error) {
            ErrorHandler_1.ErrorHandler.handle(error, 'Test Generation Command');
        }
    });
    const testStatusBarItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 90);
    ...
    context.subscriptions.push(generateTestsCommand, testStatusBarItem);
}
